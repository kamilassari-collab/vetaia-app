...
export async function POST(req: NextRequest) {
  try {
    ...
    await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      headers: { 'api-key': apiKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sender: { name: 'Leash AI', email: 'hello@leash-ai.com' },
        to: [{ email, name: `${firstName} ${lastName}` }],
        subject: 'Bienvenue sur Leash AI 🐾',
        htmlContent: `
          <div style="font-family: 'DM Sans', sans-serif; max-width: 520px; margin: 0 auto; padding: 40px 24px; color: #111D1B;">
            <div style="margin-bottom: 32px;">
              <span style="font-size: 22px; font-weight: 600; color: #111D1B;">Leash <span style="color: #0B7A6A;">AI</span></span>
            </div>
            ...
            <p style="font-size: 15px; color: #4A6460; line-height: 1.7; margin-bottom: 24px;">
              Votre compte Leash AI est activé. Vous pouvez dès maintenant vous connecter et générer vos premiers comptes-rendus vétérinaires en 30 secondes.
            </p>
