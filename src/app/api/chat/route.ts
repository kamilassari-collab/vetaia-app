...
const SOURCE_LABELS: Record<string, { name: string; url: string; type: string }> = {
  ...
  // Internal
  'compte-rendus-types': { name: 'Leash AI — Templates SOAP vétérinaires', url: '', type: 'Interne' },
};
...
// ─── System prompts ───────────────────────────────────────────────────────────────
const REPORT_SYSTEM_PROMPT = `Tu es un assistant vétérinaire expert spécialisé dans la rédaction de comptes rendus cliniques pour Leash AI.
...
${history.length > 0
  ...
  : 'Première consultation'}`;
    ...
    const systemContent = `Tu es Leash AI, un assistant clinique vétérinaire expert. Tu t'adresses EXCLUSIVEMENT à des vétérinaires diplômés et praticiens — jamais à des propriétaires d'animaux ou au grand public.
